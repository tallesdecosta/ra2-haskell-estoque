...
import Data.List (sortBy, groupBy, sortOn, isInfixOf, elemIndex)
...
updateQty agora id qtdeMudar inv =
            ...
            in if novaQtde < 0
                then Left "ERRO AO ATUALIZAR ESTOQUE: Não há estoque suficiente para realizar essa movimentação. :("  
                    ...
                    let itemAtualizado = itemEncontrado { quantidade = novaQtde }
                        ...
                        logEntry = LogEntry {
                            timestamp = agora,
                            -- Se a quantidade for positiva, é UpdateQty, senão é Remove.
...
removeltem agora id qtde inv =
    updateQty agora id (-qtde) inv -- Reutiliza a lógica de updateQty
...
historicoPorltem id logs = filter (eDoItem id) logs
    where
...
itemMaisMovimentado logs =
    ...
    where
      ...
      extrairID (LogEntry _ _ _ (Falha _)) = Nothing 
...
gameLoop inventario logs = do
    ...
    case comando of
        ...
        ["add", id, nome, qtdeStr, cat] -> do
            ...
            processarResultado agora Add comando resultado
            

        ["estoque", id, qtdeStr] -> do
            let qtde = read qtdeStr :: Int

            let resultado = updateQty agora id qtde inventario 
            let acaoTentada = if qtde > 0 then UpdateQty else Remove
            processarResultado agora acaoTentada comando resultado

        ["remover", id, qtdeStr] -> do
            let qtde = read qtdeStr :: Int

            let resultado = removeltem agora id qtde inventario 
            processarResultado agora Remove comando resultado
        ...
        ["relatorio", "item", id] -> do
            ...
            mapM_ print (historicoPorltem id logs)
            gameLoop inventario logs
    ...
    where
        processarResultado :: UTCTime -> AcaoLog -> [String] -> Either String ResultadoOperacao -> IO ()
        

        processarResultado agora acaoTentada comando (Left erroMsg) = do
        ...
        processarResultado _ _ _ (Right (novoInv, logSucesso)) = do
